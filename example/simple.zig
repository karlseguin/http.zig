...
pub fn start(allocator: Allocator) !void {
    ...
    router.get("/zwerve.jpg", zwerve);
    router.get("/index_cached.html", indexCached);
    try server.listen();
}
...
fn indexCached(req: *httpz.Request, res: *httpz.Response) !void {
    _ = req;
...
fn zwerve(req: *httpz.Request, res: *httpz.Response) !void {
    var index_file = try std.fs.cwd().openFile(req.url.path[1..], .{});
    defer index_file.close();
    res.body = try index_file.readToEndAlloc(req.arena, 100000);
}
